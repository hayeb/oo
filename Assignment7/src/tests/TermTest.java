...
public class TermTest {
	...
	public void testTimes() {
		...
		Term t2 = new Term(-2.0, 3);
		t1.times(t2);
		assertEquals("positive * negative: coefficient",-4.0, t1.getCoef(), 0.00005);
		assertEquals("positive * negative: exponent",t1.getExp(), 6);
		t1 = new Term(0.0, 3);
		t2 = new Term(2.0, 3);
		t1.times(t1);
		assertEquals("0 * something else: coefficient",0.0, t1.getCoef(), 0.00005);
		t1 = new Term(8.0, 3);
		t2 = new Term(4.0, 3);
		t1.times(t2);
		assertEquals("positive * positive: coefficient", 32.0, t1.getCoef(), 0.00005);
		assertEquals("positive * positive: exponent", 6, t1.getExp());
		t1 = new Term(-2.0, 3) ;
		t2 = new Term(-3.0, 3) ;
		t1.times(t2);
		assertEquals("negative * negative: coefficient", 6.0, t1.getCoef(), 0.00005);
