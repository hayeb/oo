...
public class Bitmap {
	...
	/**
	 * Returns if an area is only black, only white or grey.
	 * 
	 * @param x
	 ...
	 * @param width
	 *            The width (and height) of the area to be considered.
	 * @return 1 of the area is black, 2 if the area is white, 0 if the area is
	 *         grey.
	 */
	public int checkColor(int x, int y, int width) {
		boolean color = raster[x][y];
		int widthx = width + x;
		int widthy = width + y;
		if (width > 1){
			for (int i = x; i< widthx; i++) {
				for(int j = y; j < widthy; j++) {
					if (color != raster[i][j]) {
						return 0;
					}
				}
